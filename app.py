...
with st.sidebar:
    ...
    if st.button("예시 적용", use_container_width=True):
        ex = EXAMPLES[ex_name]
        st.session_state["formula_name"] = ex.get("formula", "육미지황환")
        st.session_state["chief"] = ex.get("chief", "")
        st.session_state["global_note"] = ex.get("global_note", ex.get("global", ""))
        st.session_state["goal"] = ex.get("goal", "")
        st.session_state["pulse"] = ex.get("pulse", "")
        st.session_state["tongue"] = ex.get("tongue", "")
        st.session_state["abdomen"] = ex.get("abdomen", "")
        st.session_state["bp"] = ex.get("bp", "")
        st.session_state["labs"] = ex.get("labs", "")
        st.session_state["meds"] = ex.get("meds", "")
        st.session_state["checked_safety"] = ex.get("safety", [])
        st.rerun()
