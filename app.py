...
st.warning(
    ...
    "- 전통 처방 Core가 중심이며, 기하학적 분석 모델은 보조적인 해석층입니다.\n"
    "- 실제 처방은 면허가 있는 한의사의 변증, 환자 병력, 검사 결과, 안전성 평가를 바탕으로 결정되어야 합니다."
)

st.title("☯️ 64큐브-다면체 기반 코돈·아미노산 벡터 처방 해석 대시보드")

# [수정] 한의사용 친화적 상단 공식 및 설명문 배치
st.markdown(
    "**처방 해석 구조 = 전통 처방 구조 + 동의보감 병증 해석 + 처방 중심 방향 + 주변 변화 가능성 + 보사·승강출입 균형 + 안전성 확인**\n\n"
    "<span style='font-size: 0.9em; color: #555;'>*연구자용 표현: Traditional_Core + Donguibogam_Layer + Q6_Core + H(3,4)_Extension + Polyhedron_Layer + Safety_Filter*</span>\n\n"
    "전통 처방 구조와 동의보감 병증 해석을 중심 설명층으로 두고, Q6·H(3,4)·다면체 구조는 이를 보조적으로 주석화하는 해석층으로 사용합니다.",
...
def load_data():
    ...
    clinical = pd.DataFrame({
        ...
        "clinic_structure": [
            "- 수렴·안신: 산조인\n- 자음·청열: 지모\n- 활혈·행기: 천궁\n- 조화: 감초\n- 핵심 해석: 소모된 진액을 보충하고 허열을 진정시키는 수렴형 처방 구조",
            "- 조습·건비: 창출\n- 행기·제만: 후박\n- 이기·화담: 진피\n- 조화: 감초\n- 핵심 해석: 정체된 습탁을 제거하고 기운을 강하게 돌리는 배출/전환형 처방 구조",
            "- 승양·보기: 황기, 승마, 시호\n- 건비·익기: 인삼, 백출, 감초\n- 핵심 해석: 아래로 처진 기운을 강하게 위로 끌어올리는 발산/상승형 처방 구조",
    ...
    # [수정] 임산부 "안전" 표기를 임상적 안전 표현으로 순화
    saf_flag = "일반 용량 범위 내 특이 주의 제한"
    safety = pd.DataFrame({
        "herb_name": ["산조인", "지모", "천궁", "감초", "창출", "후박", "진피", "황기", "인삼", "백출", "당귀", "승마", "시호", "숙지황", "산수유", "산약", "복령", "목단피", "택사"],
        ...
        "pregnancy_flag": [saf_flag, saf_flag, "주의권고", saf_flag, saf_flag, "신중투여", saf_flag, saf_flag, saf_flag, saf_flag, "주의권고", saf_flag, saf_flag, saf_flag, saf_flag, saf_flag, saf_flag, "금기추정", saf_flag],
...
if analyze_btn:
    ...
    # ------------------------------------------
    # 5. 보사·승강출입 균형 (구 다면체 시각화)
    # ------------------------------------------
    with tabs[4]:
        st.subheader("⚖️ 5. 보사·승강출입 균형 패널")
    ...
    with tabs[7]:
        ...
        # [수정] 영문 컬럼명을 직관적인 한글명으로 렌더링
        display_safety = formula_safety[["herb_name", "drug_interaction_flag", "pregnancy_flag", "liver_kidney_flag", "evidence_note"]].rename(
            columns={
                "herb_name": "약재명",
                "drug_interaction_flag": "약물 병용 주의",
                "pregnancy_flag": "임신·수유 관련",
                "liver_kidney_flag": "간·신장 관련",
                "evidence_note": "주의 근거/확인 사항"
            }
        )
