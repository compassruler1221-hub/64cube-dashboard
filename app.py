# app.py
# 한의사용 처방·침구·뜸 보조 대시보드 v99-safe + 상한론·Q6 한의학 연구자 해설·텐세그리티·플라토닉 환론 엔진
...
# ---------------- 8번: 상한론 원전 감별 주석 ----------------
SHANGHAN_REFERENCE_ROWS = [
    {"처방":"계지탕", "상한론 위치":"태양중풍 계열", "핵심 증후":"발열·오풍·자한·맥완", "감별 포인트":"땀이 나고 바람을 싫어하면 영위불화·표허 쪽으로 본다.", "앱 해석":"표리 출입이 약하게 열린 상태. 해표보다 조영위·완충축 확인."},
    {"처방":"마황탕", "상한론 위치":"태양상한 계열", "핵심 증후":"오한·무한·신통·맥부긴·해수/천", "감별 포인트":"땀이 없고 표실·한사가 뚜렷하면 발한해표를 검토한다.", "앱 해석":"폐·표층의 닫힘. 소통·승양축을 쓰되 심계·혈압·불면 안전 확인."},
    {"처방":"갈근탕", "상한론 위치":"태양병 항배강 계열", "핵심 증후":"목·어깨 뻣뻣함, 오한·두통·표증", "감별 포인트":"태양표증에 경항부 긴장이 중심이면 갈근탕 방향을 본다.", "앱 해석":"표층 긴장과 경항부 축의 막힘. 폐·대장·방광경 소통 후보 확인."},
    {"처방":"소시호탕", "상한론 위치":"소양병·반표반리 계열", "핵심 증후":"왕래한열·흉협고만·구고·인건·목현·심번희구", "감별 포인트":"구역과 발열이 있어도 심하비가 중심이면 바로 시호탕으로 가지 않는다.", "앱 해석":"간담 소양축·흉협축의 왕래 불리. 소통·완충축 확인."},
    {"처방":"반하사심탕", "상한론 위치":"149조: 심하비 감별", "핵심 증후":"상한 5~6일 뒤 구역·발열, 명치 아래가 그득하나 아프지 않음", "감별 포인트":"但滿而不痛者，此爲痞，柴胡不中與之，宜半夏瀉心湯. 시호탕 부적합, 반하사심탕 방향.", "앱 해석":"중초 비위 노드 막힘 + 한열착잡 + 위기상역. CV12·PC6·ST36·SP4·CV13 후보."},
    {"처방":"대시호탕", "상한론 위치":"소양양명 합병·실증 계열", "핵심 증후":"흉협고만·심하급·구역·변비·복부 긴장", "감별 포인트":"소양증에 양명 실체·변비·복부 긴장이 결합하면 소시호탕보다 강한 방향을 검토한다.", "앱 해석":"소통축 + 배출축. 허약·설사·탈수·임신 가능성은 강하게 경고."},
    {"처방":"소함흉탕", "상한론 위치":"소결흉·담열결흉 계열", "핵심 증후":"흉민·심하부 막힘·담열·명치 답답함", "감별 포인트":"심하부가 아프고 담열·흉격 막힘이 있으면 반하사심탕 심하비와 구별한다.", "앱 해석":"흉격과 중초 사이의 담열 막힘. 청열화담·개결축 설명."},
    {"처방":"대함흉탕", "상한론 위치":"결흉·수열결흉 계열", "핵심 증후":"심하부가 단단하고 아픔, 실증 급성 복증 가능", "감별 포인트":"단순 그득함이 아니라 통증·단단함·급성 복증이면 응급 감별 우선.", "앱 해석":"강한 사하·축수 방향. 앱에서는 경고/감별 설명용으로만 표시."},
    {"처방":"백호가인삼탕", "상한론 위치":"양명기분열·진액 손상 계열", "핵심 증후":"대열·대한·대갈·맥홍대·피로/진액 손상", "감별 포인트":"실열과 진액 손상이 중심이면 청열생진 방향을 본다.", "앱 해석":"강한 열 편향 + 진액 소모. 뜸 보류·청열/생진 설명."},
    {"처방":"진무탕", "상한론 위치":"소음·양허수범 계열", "핵심 증후":"냉감·무력·부종·설사·소변불리·어지럼", "감별 포인트":"수습정체라도 냉감·양허가 뚜렷하면 오령산과 구분한다.", "앱 해석":"신·비 양허와 수습축 정체. 온양이수 방향을 안전하게 설명."},
]

SHANGHAN_149_DIFF_ROWS = [
    {"분기":"구역+발열+흉협고만", "복진":"흉협부 답답함이 중심", "판단":"소양 반표반리", "처방 방향":"소시호탕 또는 소양 화해 방향"},
...
def shanghan_rows_for_formula() -> List[Dict[str, str]]:
    rows = [r for r in SHANGHAN_REFERENCE_ROWS if r["처방"] == formula_name]
    if rows:
        return rows
    related = []
    related_names = [x.strip() for x in formula.get("감별 처방", "").split(",") if x.strip()]
    for r in SHANGHAN_REFERENCE_ROWS:
        if r["처방"] in related_names:
            rr = dict(r)
            rr["처방"] = rr["처방"] + " — 감별 후보"
            related.append(rr)
    return related or [{"처방":formula_name, "상한론 위치":"직접 대응 없음", "핵심 증후":"현재 선택 처방은 상한론 핵심 처방군보다 후대 방제·잡병 해석에 더 가깝게 사용될 수 있음", "감별 포인트":"상한론식으로는 표리·한열·허실·삼음삼양 위치를 보조 주석으로만 사용", "앱 해석":"황제내경·동의보감 편제와 처방 방향 6축을 우선 적용"}]

def shanghan_chart_text() -> str:
    current = shanghan_rows_for_formula()[0]
    return f"""[상한론 감별 주석]
선택 처방: {formula_name}
전통 변증: {formula['전통 변증']}
상한론 연결: {current['상한론 위치']}
핵심 증후: {current['핵심 증후']}
감별 포인트: {current['감별 포인트']}
앱 해석: {current['앱 해석']}

149조 핵심:
상한 5~6일 뒤 구역질과 발열이 있어도, 명치 아래가 그득하지만 아프지 않으면 심하비로 보며 시호탕을 바로 쓰지 않고 반하사심탕 방향을 검토한다. 반대로 심하부가 단단하고 아프면 결흉·급성 복증 감별을 우선한다.

임상 안전 문구:
이 내용은 원전 조문을 현대 임상 설명용으로 주석화한 것이며 자동 처방 지시가 아니다. 실제 처방 여부와 용량, 침구·뜸 적용은 한의사가 문진·맥진·설진·복진·검사값·복용약을 종합해 결정한다.
"""

# ---------------- 12번: 연구자용 Q6/H(3,4)의 한의학 상세 해설 ----------------
RESEARCH_TCM_AXIS_ROWS = [
    {"비트/축":"bit0 초효", "한의학 해석":"내부 동기·초발 병기", "6축 연결":"소통·전환축", "임상 질문":"증상이 막힘에서 시작했는가, 허약에서 시작했는가?", "주의":"초효는 생물학적 염기서열이 아니라 64상태 모델의 첫 변환축"},
    {"비트/축":"bit1 2효", "한의학 해석":"중초 반응·비위 운화", "6축 연결":"보충축/배출·이수축", "임상 질문":"먹으면 더부룩한가, 설사·습담이 있는가?", "주의":"비위 반응을 수치로 확정하지 않음"},
    {"비트/축":"bit2 3효", "한의학 해석":"하초·수습·정혈 기반", "6축 연결":"수렴·안정축/배출·이수축", "임상 질문":"소변·부종·요슬·도한·냉감은 어떤가?", "주의":"신허·수습의 설명축"},
    {"비트/축":"bit3 4효", "한의학 해석":"흉협·표리 경계", "6축 연결":"소통·전환축/완충·조화축", "임상 질문":"왕래한열, 흉협고만, 구역, 심하비가 있는가?", "주의":"소양/중초 감별을 돕는 축"},
    {"비트/축":"bit4 5효", "한의학 해석":"상초·심폐·신명 반응", "6축 연결":"승양·상승축/수렴·안정축", "임상 질문":"심계·불면·흉민·기침·상열감이 있는가?", "주의":"안신/해표/상열 구분용"},
    {"비트/축":"bit5 상효", "한의학 해석":"전체 방향의 귀결·전신 정렬", "6축 연결":"완충·조화축", "임상 질문":"처방 후 전체 균형이 안정되는가, 한쪽으로 치우치는가?", "주의":"최종 치료효과를 보장하는 축이 아님"},
]

Q6_H34_TCM_ROWS = [
    {"구조":"Q6 64큐브", "수학적 뜻":"64개 꼭짓점, 한 번에 한 비트만 바뀌는 6차원 하이퍼큐브", "한의학적 번역":"64가지 변증 상태를 6개 처방 방향축의 조합으로 보는 Core 모델", "임상 사용":"현재 상태에서 한 축만 바꾸면 어떤 감별 처방/혈위 후보로 이동하는지 설명"},
    {"구조":"Q6 edge", "수학적 뜻":"한 비트 변화", "한의학적 번역":"한 병기축의 변화: 예를 들어 습담→소통, 허열→수렴, 하함→승양", "임상 사용":"처방 변경·가감·침구 자극 방향이 과도하게 여러 축을 흔들지 않는지 점검"},
    {"구조":"H(3,4)", "수학적 뜻":"3자리×4상태 codon-like 확장 그래프", "한의학적 번역":"하초·중초·상초 3층 각각에서 네 방향 변화 가능성을 보는 넓은 감별망", "임상 사용":"Q6보다 넓게 주변 처방군과 감별 후보를 보여주는 연구자용 지도"},
    {"구조":"n ↔ 63-n", "수학적 뜻":"중심반전·상보쌍", "한의학적 번역":"음양 상보, 한열·허실·표리의 반대편을 같이 확인하는 원리", "임상 사용":"보하는 처방을 쓸 때 사해야 할 정체가 있는지, 청열할 때 허한이 있는지 확인"},
    {"구조":"9의 배수 축", "수학적 뜻":"0·9·18·27·36·45·54·63의 대각적 질서", "한의학적 번역":"64상태 안에서 음양이 교차하는 중심 관찰축", "임상 사용":"시작·전환·정지·회복 같은 병기 흐름을 교육용으로 설명"},
]

def current_formula_research_rows() -> List[Dict[str, str]]:
    axis_meaning = {
        "보충축":"허약·기혈음양 부족을 보태는 방향. 팔진탕·십전대보탕·육미지황환 등에서 중심이 된다.",
        "수렴·안정축":"흩어진 기운·도한·불면·심계를 안으로 모으는 방향. 산조인탕·귀비탕·천왕보심단 등에서 중요하다.",
        "승양·상승축":"처진 기운과 중기하함을 들어 올리는 방향. 보중익기탕·마황탕류의 발산/승양과 구별해야 한다.",
        "배출·이수축":"습담·수습·부종·정체를 밖으로 움직이는 방향. 오령산·이진탕·진무탕·대시호탕 일부가 연결된다.",
        "소통·전환축":"기체·흉협·심하비·구역·상역의 방향을 돌리는 축. 반하사심탕·소시호탕·평위산·소요산에서 핵심이다.",
        "완충·조화축":"한열·허실·보사의 치우침을 조절하는 축. 반하사심탕·소시호탕·산조인탕 등에서 충돌을 완충한다.",
    }
    return [{"현재 처방축":axis, "한의학적 의미":axis_meaning.get(axis, "처방 방향 보조축"), "Q6 해석":"이 축 하나의 변화가 Q6 edge 하나에 해당하는 것으로 주석화", "H(3,4) 해석":"상·중·하초 중 어느 층에서 이 축이 나타나는지 넓게 감별"} for axis in formula.get("처방 방향축", [])]

def research_tcm_note() -> str:
    codon = n_to_codon(9)
    return f"""[연구자용 Q6/H(3,4) 한의학 해설]
선택 처방: {formula_name}
처방 변증: {formula['전통 변증']}
처방 방향축: {', '.join(formula['처방 방향축'])}

Q6 해석:
Q6 64큐브는 64가지 변증 상태를 6개 처방 방향축의 조합으로 보는 연구자용 정보기하학 모델이다. 한 edge는 한 병기축만 바뀌는 변화로 보며, 처방·가감·침구 후보가 어느 축을 움직이는지 설명한다.

H(3,4) 해석:
H(3,4)는 하초·중초·상초라는 3층과 각 층의 네 방향 변화를 넓게 보는 감별망으로 사용한다. Q6가 핵심 변환축이라면 H(3,4)는 주변 처방군과 감별 후보를 더 넓게 펼쳐 보는 지도다.

한의학적 사용:
이 모델은 약재가 실제 코돈·아미노산·유전자를 조절한다는 뜻이 아니다. 전통 병기와 처방 방향을 64상태·상보쌍·중심평형이라는 언어로 주석화한 교육·연구 보조층이다. 예시로 9번은 효순서 {n_to_hyo_bits(9)}, codon {codon}, 표준 유전암호상 {GENETIC_CODE[codon]}로 표시되지만, 이는 생물학적 인과 증명이 아니라 구조적 병렬 매핑이다.
"""
