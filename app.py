...
def load_data():
    ...
    return formulas, donguibogam, neijing, polyhedrons, herbs, safety, vectors

df_formulas, df_donguibogam, df_neijing, df_polyhedrons, df_herbs, df_safety, df_vectors = load_data()
...
if analyze_btn:
    ...
    with tabs[0]:
        ...
        st.markdown(f"자세한 방향은 5번, 6번 탭의 내용을 참고하세요.")
    ...
    with tabs[3]:
        ...
        if selected_formula_name == "육미지황환":
            st.success("**[정합 소견]**\n- 맥: 침(沈), 세(細), 약(弱), 삭(數) 경향\n- 설: 홍(紅) 또는 담홍(淡紅), 소태(少苔), 무태, 건조\n- 증상: 도한, 오심번열, 요슬산연, 피로, 구건")
            st.warning("**[주의 소견]**\n- 설태가 후니(厚膩)함 (식체·습담)\n- 복부 냉감 및 잦은 설사 (위장 허약)")
            st.error("**[재검토 요망 소견]**\n- 실열·습열 소견이 뚜렷함\n- 설태 황니\n- 맥이 실·활·삭하고 열상이 강함")
        elif selected_formula_name == "보중익기탕":
            st.success("**[정합 소견]**\n- 맥: 허(虛), 약(弱), 완(緩), 무력 경향\n- 설: 담(淡), 치흔(齒痕), 백태 경향\n- 증상: 피로, 기단, 식욕저하, 내장하수감, 자한")
            st.warning("**[주의 소견]**\n- 상열감, 고혈압, 심계, 불면")
            st.error("**[재검토 요망 소견]**\n- 실열, 습열, 뚜렷한 식적이나 담음")
        elif selected_formula_name == "산조인탕":
            st.success("**[정합 소견]**\n- 맥: 세(細), 약(弱), 현세(弦細) 경향\n- 설: 담홍 또는 홍, 소태\n- 증상: 허번불면, 심계, 불안, 피로")
            st.warning("**[주의 소견]**\n- 낮 졸림, 진정제 병용, 간·신장 기능 저하")
            st.error("**[재검토 요망 소견]**\n- 실열성 불면, 담열이 뚜렷한 불면")
        else:
    ...
    with tabs[4]:
        ...
        if selected_formula_name == "육미지황환":
            ...
            **[검토 가능한 혈위군]**
            - 비·위 경락축
            - 중기 보강 축 / 승양 축 / 기허 피로 조절 축
            """)
        elif selected_formula_name == "산조인탕":
            st.markdown("""
            **[침 치료 방향]**
            - 허번불면·안신 방향 확인 / 수렴·내부 안정 방향
            - 과도한 자극보다 완만한 조절 방향
            - 수면제·진정제 병용 환자는 졸림·반응 저하 확인

            **[검토 가능한 혈위군]**
            - 심·간·담 축 / 안신 축 / 음혈 보강 축 / 수면 안정 축
            """)
        else:
            st.markdown("- 변증 방향에 따른 평보평사 위주 및 기혈 조절 방향 검토")
    ...
    with tabs[5]:
        ...
        if selected_formula_name == "육미지황환":
            ...
            **🔴 뜸 주의/재검토:** 상열감, 고혈압, 두근거림, 불면, 안면홍조 등 상승 기운이 이미 뚜렷한 경우
            """)
        elif selected_formula_name == "산조인탕":
            st.markdown("""
            **🟢 뜸 검토 가능:** 허한성 불면, 냉감 동반, 양허성 피로
            
            **🔴 뜸 주의/재검토:** 열감, 번조, 구건, 불면이 열성으로 뚜렷한 경우, 진정제 병용으로 반응 저하가 있는 경우
            """)
        else:
            st.markdown("- 환자의 허열/실열 양상 및 화상 위험도에 따라 온열 자극 여부 결정 요망")
    ...
    with tabs[6]:
        ...
        st.warning("본 패널은 처방의 중심 방향에서 벗어날 수 있는 주변 변화 가능성을 한의학적 변증·안전성 언어로 번역한 보조 설명층입니다. 약재가 실제 유전암호를 조절한다는 뜻이 아닙니다.")

        st.markdown("""
        ### 🔑 한의사용 해석
        H(3,4) Extension은 한의사에게는 복잡한 그래프가 아니라 **처방의 중심 변증 방향 주변에서 어떤 주의점이 생길 수 있는지 보는 확장 지도**로 이해하면 됩니다.
        """)
        ...
        if selected_formula_name == "보중익기탕":
            ...
            #### 3. 혈당·혈압 변동 방향
            - 인삼, 감초 등과 관련하여 당뇨약·혈압약 복용 여부 확인
            #### 4. 약물 병용 주의 방향
            - 감초: 이뇨제·혈압약 병용 확인
            - 당귀: 항응고제/항혈소판제 병용 확인
            ...
            #### 3. 허열 완충 방향
            - 도한, 오심번열, 열감, 수면 변화 확인 (실열 뚜렷 시 재검토)
            #### 4. 약물 병용 주의 방향
            - 목단피: 항응고제/항혈소판제 복용 확인
            - 당뇨약 복용 시 혈당 변화 확인
    ...
    with tabs[7]:
        ...
        st.markdown("""
        ### 🔑 한의사용 해석
        다면체 분석은 도형 자체를 보라는 뜻이 아니라, **처방이 어느 방향으로 치우치고, 어느 축에서 균형을 잡는지**를 보는 설명 방식입니다.
        ...
        if selected_formula_name == "육미지황환":
            ...
            #### 2. 승강출입
            - 저장·수렴·보존 방향이 강함 (급격한 발산이나 공하 방향은 낮음)
            #### 3. 한열허실
            ...
            #### 2. 승강출입
            - 상승·발산 방향이 강함 (중기를 들어 올리는 방향으로 해석)
            #### 3. 한열허실
            - 허증 중심 (실열·습열이 뚜렷한 경우 재검토 필요, 상열감 반응 확인)
            """)
        elif selected_formula_name == "산조인탕":
            st.success("핵심 해석: 수렴·안신·내부 안정 방향이 강한 처방")
            st.markdown("""
            #### 1. 보사 균형
            - 보혈·자음과 안정 방향 (과도한 발산을 줄이고 내부 안정)
            #### 2. 승강출입
            - 내부 수렴 방향이 강함 (정신적 번조를 가라앉히는 방향)
            #### 3. 한열허실
            - 허번불면 방향에서 검토 (실열성 불면과 구분 필요)
            """)

    # ------------------------------------------
    with tabs[8]:
        st.subheader("🧬 9. Q6 64큐브 Core 주석 패널")
        st.info("Q6 layer는 처방의 전통적 방향성을 6비트 상태공간의 언어로 주석화(Annotation)하는 연구자용 층입니다.")
