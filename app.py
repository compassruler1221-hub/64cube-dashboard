...
# ==========================================
# 1. 풍성해진 빅데이터 베이스 (엑셀 연동 준비 완료)
# ==========================================
@st.cache_data
def load_data():
    # 향후 pd.read_csv('formulas.csv') 등으로 교체될 수 있도록 구조화된 데이터
    ...
    herbs = pd.DataFrame({
        "formula_id": [
            "F001", "F001", "F001", "F001", 
            "F002", "F002", "F002", "F002",
            "F003", "F003", "F003", "F003", "F003", "F003", # 보중익기탕 일부
            "F004", "F004", "F004", "F004", "F004", "F004"  # 육미지황환
        ],
        "herb_name": [
            "산조인", "지모", "천궁", "감초", 
            "창출", "후박", "진피", "감초",
        ...
        "dose_range": [
            ...
            "15-20g", "9-12g", "9-12g", "6-9g", "3-6g", "3-6g",
    ...
    safety = pd.DataFrame({
        "herb_name": ["산조인", "천궁", "감초", "창출", "후박", "인삼", "승마", "시호", "숙지황", "목단피"],
        "drug_interaction_flag": ["수면제, 진정제", "항응고제, 항혈소판제", "이뇨제, 혈압약", "", "", "혈당강하제, 혈압약", "", "", "", "항응고제"],
        "pregnancy_flag": ["", "주의권고", "", "", "신중투여", "", "", "", "", "금기추정"],
        "liver_kidney_flag": ["대량사용시 간부담", "", "장기복용시 신장/혈압 주의", "", "", "", "", "간효소 수치 주의", "소화기계 부담", ""],
        "evidence_level": ["임상확인필요", "문헌보고", "문헌보고", "전통금기", "주의권고", "문헌보고", "전통주의", "임상확인필요", "전통주의", "문헌보고"],
        "evidence_note": ["과다 복용 시 간효소 수치 상승", "혈소판 응집 억제 작용", "위알도스테론증 유발 가능", "음허내열 금기", "동물실험 자궁수축", "과량 복용 시 두근거림/혈압상승", "상열감 환자 주의", "장기 복용 시 간부담", "소화불량/설사 유발 가능", "혈류 순환 촉진"]
    })
    
    vectors = pd.DataFrame({
        "herb_name": ["산조인", "천궁", "창출", "황기", "인삼", "숙지황", "산수유"],
        "functional_module": ["수면조절, 진정", "염증 완충, 혈류 순환", "위장관 운동, 습탁 제거", "대사 경로 활성화", "ATP 생성 촉진", "구조 단백질 합성/보존", "체액 손실 방지"],
        "q6_vector_type": ["보존형 변화", "완충형 변화", "급진 전환형 변화", "비정상 연장형 변화", "급진 전환형 변화", "보존형 변화", "수렴형 완충"],
        "biochemical_interpretation": ["극도 긴장을 완화하고 내부 안정 모듈 강화", "과도한 편향을 완충하여 제약 완화", "정체된 상태를 풀고 흐름 회복", "기저 대사량을 끌어올려 에너지 상태 연장", "미토콘드리아 대사 사이클 가속", "세포 내 물질 손실을 막고 구조적 안정성 확보", "체액 증발을 막는 물리적 장벽 강화"]
    })
...
if analyze_btn:
    ...
    with tab1:
        ...
        st.dataframe(formula_herbs, use_container_width=True, hide_index=True)
    ...
    with tab2:
        ...
        **💡 64큐브 방향성 요약 ({selected_formula_name} 기준)**
        ...
        if not formula_vectors.empty:
            ...
            st.info("현재 선택된 처방의 정밀 벡터 매핑 데이터가 업데이트 중입니다.")
