...
# [NEW] 진맥/설진 입력 (정합도 판정에 사용됨)
st.sidebar.subheader("진맥 및 설진 소견")
col_p1, col_p2 = st.columns(2)
with col_p1:
    mac_force = st.sidebar.selectbox("맥력(Pulse)", ["선택안함", "허(虛)맥", "실(實)맥", "약(弱)맥"])
with col_p2:
    tongue_color = st.sidebar.selectbox("설질(Tongue)", ["선택안함", "담(淡)", "홍(紅)", "암(暗)"])
tongue_coat = st.sidebar.selectbox("설태(Coating)", ["선택안함", "백태(白)", "황니태(黃膩)", "소태/무태"])
...
cond_liver = st.sidebar.checkbox("간 기능 저하 또는 간질환")
cond_kidney = st.sidebar.checkbox("신장 기능 저하 또는 신장질환")
...
st.sidebar.subheader("임상 검사값 입력")
