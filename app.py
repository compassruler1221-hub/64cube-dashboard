...
def load_data():
    ...
    return formulas, df_polyhedrons, df_neijing, donguibogam, clinical, herbs, safety, vectors
...
if analyze_btn:
    ...
    with tabs[1]:
        ...
        st.error("**[해석 주의] 동의보감 해석 패널은 처방의 전통적 병증 이해, 양생, 장부·기혈·담음·허실·한열 관점을 한국 한의학 원전 맥락에서 정리하는 층입니다. 이는 현대 유전암호를 직접 설명했다는 뜻이 아니라, 전통 한의학의 병증·처방 해석 언어를 중심 설명층에 보강하는 원전 주석층입니다.**")
    ...
    with tabs[2]:
        ...
        translate_df = pd.DataFrame([
            ...
            {"연구자용 표현": "H(3,4) Extension", "한의사용 해석": "처방 좌표 주변의 변화 가능성을 넓게 참고하는 보조 지도"},
    ...
    with tabs[4]:
        ...
        if selected_formula_name == "육미지황환":
            ...
            #### 3. 한열허실
            - 허증 중심
            - 실열·습열이 뚜렷한 경우 재검토 필요, 상열감 반응 확인
            #### 4. 임상 확인
