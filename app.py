...
def load_data():
    ...
    # [최종수정] ATP 생성, 면역 활성화, 혈당 강화 등 생리효과 단정 표현을 '해석축'으로 전면 교체
...
if analyze_btn:
    ...
    with tab2:
        ...
        # [최종수정] 유전암호 패널에도 해석 주의문(방어선) 반복 배치
        st.error("**[해석 주의] 약재-코돈-아미노산 매핑은 약재가 실제 유전암호를 직접 변화시킨다는 뜻이 아니라, 약재의 전통적 작용 방향을 64큐브 코돈-아미노산 물성 벡터 언어로 주석화한 해석 모델입니다.**")
...
"""
        ...
        if selected_formula_name == "육미지황환":
            ...
            <div style="background-color:#eaf3ff; padding:20px; border-radius:10px; line-height:1.8; color:#1e293b;">
                ...
                전통 한의학적 해석에서는 자음, 보신, 구조물질 보충의 방향으로 해석할 수 있으며, 64큐브 다면체 해석에서는 <b>숙지황·산약·산수유의 보존/보충 중심축</b>과 <b>복령·택사·목단피의 수습/허열 완충축</b>이 3:3으로 마주 보는 RD 안정화 구조로 해석됩니다.<br><br>
                {warning_text}
            </div>
            """
