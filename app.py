...
def load_data():
    ...
    vectors = pd.DataFrame({
        "herb_name": ["산조인", "지모", "천궁", "감초", "창출", "후박", "진피", "황기", "인삼", "백출", "당귀", "승마", "시호", "숙지황", "산수유", "산약", "복령", "목단피", "택사"],
        "codon": ["UUU (Uracil)", "UCU (Uracil)", "UAU (Uracil)", "UGG (Uracil)", "CUU (Cytosine)", "CCU (Cytosine)", "CAU (Cytosine)", "AUG (Adenine)", "CGU (Cytosine)", "AUU (Adenine)", "ACU (Adenine)", "AAU (Adenine)", "AGU (Adenine)", "GUU (Guanine)", "GCU (Guanine)", "GAU (Guanine)", "GGU (Guanine)", "UGU (Uracil)", "CAA (Cytosine)"],
        ...
        "functional_module": ["소수성 안정화 (수면조절)", "친수성 열 완충 (해열)", "방향족 고리 구조 완충 (혈류)", "전신 조화, 점막 완충", "강력한 소수성 드라이브", "구조적 꺾임(Kink) 유도", "양전하 결합 활성화 (기행)", "대사 경로의 시작 신호", "산화질소 및 ATP 생성 촉진", "소수성 코어 형성", "친수성 수소결합 유지", "극성 아미노산 네트워크", "효소 활성부위 유연성 제공", "강력한 구조 단백질 뼈대 (자음)", "비극성 메틸기 완충 (체액 보존)", "음전하 기반 수분 포획 (점막 재생)", "가장 작은 분자구조 유연성", "이황화 결합 조절 (순환 촉진)", "친수성 측사슬 극성 배출 (이뇨)"],
        ...
        "biochemical_interpretation": ["극도 긴장을 완화하고 소수성 모듈 강화", "열성 편향을 친수성 수소결합 장벽으로 완충", "방향족 구조의 전자를 활용해 과도한 편향 완충", "다양한 아미노산들의 충돌을 거대 분자로 중화", "정체된 상태를 강력한 소수성 반발력으로 회복", "단백질 2차 구조의 꺾임을 통해 과도한 수축 이완", "양전하를 띤 표면 구조를 통해 완만한 균형 조절", "단백질 합성의 시작을 알리며 기저 대사량을 끌어올림", "혈관 확장 신호전달을 통해 미토콘드리아 대사 가속", "세포막 내부의 에너지 흡수율 및 구조 보존", "혈액 내 유효 물질의 수화 상태 유지", "수용성 방어 기전을 전신으로 활성화", "간 대사 효소계의 과부하를 유연하게 완충", "세포 내 물리적 뼈대를 형성하여 물질 손실 방지", "비극성 장벽을 통해 체액 증발을 막는 물리적 장벽 강화", "음전하를 띠어 위장관 점막의 수분을 강하게 포획", "공간적 제약 없이 잉여 수분을 배출 경로로 유도", "말초 혈관 내 단백질 결합을 풀어 순환 정체 해소", "강한 친수성 삼투압을 통해 신장 여과압 조절"]
    })
...
if analyze_btn:
    ...
    with tab1:
        ...
        # [보완 1 반영: 적응증/변증 정합도 패널 원천 탑재]
        # 입력된 진단명과 처방의 전통 변증 방향을 대조하는 안전망
        if patient_symp:
            st.error(
                f"**⚠️ [임상 정합도 확인 알림]**\n\n"
...
- 안전성 체크: {'관련 주의 및 추가 검토 필요' if safety_alerts else '특이사항 없음'}
        ...
        # [보완 2, 3 반영] 
        # 효과 단정 "설계되었습니다" ➔ 임상적 "해석/설명됩니다"로 완벽 톤다운
        # st.info 파란 박스 대신 마크다운이 완벽하게 깨짐 없이 렌더링되는 HTML 섀도우 박스 인젝션
        if selected_formula_name == "육미지황환":
            ...
            warning_text = ""
            if symptom_warnings:
                warning_text = f"다만, 현재 입력된 정보(당뇨약 복용, 만성 소화불량, 간/신장 저하 등)가 체크되어 있으므로, <b>{', '.join(symptom_warnings)}</b>를 진료 과정에서 세밀하게 확인해야 합니다."
            else:
            ...
            # 기타 처방 톤다운 및 마크다운 깨짐 없는 기본 수려한 HTML 박스 처리
